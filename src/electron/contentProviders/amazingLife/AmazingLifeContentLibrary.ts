...
export class AmazingLifeContentLibrary {
  ...
  public static async getContentLibrary(): Promise<ContentLibraryCategory[]> {
    ...
    const accessToken = await AmazingLifeConnect.ensureValidToken()
    if (!accessToken) {
      console.error("Not authenticated with APlay or unable to refresh token")
      return []
    }
  ...
  public static async checkMediaLicense(mediaId: string): Promise<string | null> {
    ...
    const accessToken = await AmazingLifeConnect.ensureValidToken()
    if (!accessToken) {
      console.warn("Not authenticated with APlay or unable to refresh token, cannot check license")
      return null
    }
